...
GNB::GNB() {
   ...
   */
  left_means = ArrayXd(4);  // 4 for s, d, s_dot, d_dot
  left_means << 0,0,0,0;

  left_sds = ArrayXd(4);  // 4 for s, d, s_dot, d_dot
  left_sds << 0,0,0,0;

  left_prior = 0;

  keep_means = ArrayXd(4);  // 4 for s, d, s_dot, d_dot
  keep_means << 0,0,0,0;

  keep_sds = ArrayXd(4);  // 4 for s, d, s_dot, d_dot
  keep_sds << 0,0,0,0;

  keep_prior = 0;

  right_means = ArrayXd(4);  // 4 for s, d, s_dot, d_dot
  right_means << 0,0,0,0;

  right_sds = ArrayXd(4);  // 4 for s, d, s_dot, d_dot
  right_sds << 0,0,0,0;
...
void GNB::train(const vector<vector<double>> &data, 
  ...
  // Calculate left, keep, right probabilities
